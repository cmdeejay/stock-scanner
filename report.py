...
class StockScaner:
    def __init__(self): 
        ...
        self.newFile = r'\Users\min\Development\stockscanner\files\output.xlsx'
        self.today = date.today()
        self.start = date.today() + relativedelta(months=-3)
        self.exportList = pd.DataFrame()
        self.solution = pd.DataFrame()
        self.symbols = []
        self.table = pd.read_csv(self.filePath, names=['Order', 'symbol', 'Name', 'Yoy', 'small cap', 'big cap'], header=None, index_col=None, engine='python')
